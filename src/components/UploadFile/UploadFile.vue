    ...
    <div class="download-wrapper__files" v-if="uploadFile.length">
      ...
      <div class="files-item" v-for="(file, index) in uploadFile" :key="index">
        ...
        <div class="files-item__convert">
          <h6>в</h6>
          <div class="option-block" :class="{'open': isOpenList}">
            <p class="option-block__current" @click="isOpenList = !isOpenList">{{ convertTo }}</p>
            <div class="option-block__list" :class="{ 'open-list':  isOpenList}">
              <p
                  class="list__item"
                  v-for="(item, index) in convertList"
                  @click="convertTo = item; isOpenList=false"
                  :key="index"
              >
                {{ item }}
              </p>
